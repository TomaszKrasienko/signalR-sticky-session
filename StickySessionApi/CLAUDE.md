...
- K8s deploy (`deploy/`): `configmap.yaml` holds `Redis__Enabled`/`Redis__ConnectionString`, consumed by `deployment.yaml` via `envFrom`. `redis-deployment.yaml` runs Redis (Deployment+Service, no persistence, pub/sub only). Toggle backplane on/off: edit configmap value, `kubectl apply`, then `kubectl rollout restart deployment/stickysessionapi`.
